...
function getLabels(displayLabels) {
    ...
    for (let i = 0; i < displayLabels.length; i++) {
        ...
        let docSubCat = displayLabels[i].slice(22,36);
        ...
        } else if (docCat === 'product-label' && docSubCat === 'serial-numbers') {
            $addDocNumber.textContent = docNum;
    ...
    document.getElementById('add-to-order').onclick = function() {
        ...
        // // if no quantities were entered, cancel action (return)
        ...
        // iterate over all labels with quantities
        for (let i = 0; i < displayLabels.length; i++) {
            ...
            // holds the DOC number
            let docNum = displayLabels[i].slice(displayLabels[i].length-11, displayLabels[i].length-4);
            let docCat = displayLabels[i].slice(7,21);
            let docSubCat = displayLabels[i].slice(22,36);
            
            // if the quantity entered were wrong: smaller than 0 or bigger than (2 or 20), disregard action
            if ((docCat !== 'product-labels' && (qtyAsNum > 0 && qtyAsNum <= 20)) ||
            (docCat === 'product-labels' && (docSubCat !== 'serial-numbers' && (qtyAsNum > 0 && qtyAsNum <= 2)) ||
            (docSubCat === 'serial-numbers' && (qtyAsNum === 1)))) {
        ...
        if (allLabels.length) {
            document.querySelector('#submit-now').textContent = 'Please select another label category or submit order now.';
